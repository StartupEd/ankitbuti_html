import { selfAuthoredBlogs } from '@/data/selfAuthoredBlogs';

export default function SelfAuthoredBlogs() {
  return (
    <section>
        ...
        {selfAuthoredBlogs.map((b) => (
          ...
          >
            ...
            <div className="space-y-1.5 mb-2">
              <div className="text-[13px] font-medium text-gray-600 dark:text-gray-400">{b.site}{b.date ? ` · ${b.date}` : ''}</div>
              <h3 className="font-semibold text-gray-800 dark:text-gray-100">
                <a className="before:absolute before:inset-0" href={b.url} target="_blank" rel="noopener noreferrer">
                  {b.title}
                </a>
              </h3>
            </div>
            <p className="text-sm text-gray-600 dark:text-gray-400">{b.excerpt}</p>
          </article>
        ))}
      </div>
    </section>
